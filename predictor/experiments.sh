...
for n in 1 5 21; do
    for w in uniform distance; do
        for m in euclidean manhattan; do
            $PROGRAM main.py -knn $n $w $m $DATA
        done
    done
done
...
for d in 3 10 None; do
    for c in gini entropy; do
        for l in 1 5 20; do
            $PROGRAM main.py -dtc $d $c $l $DATA
        done
    done
done
...
for n in 10 100; do
    for f in sqrt log2; do
        for b in True False; do
            $PROGRAM main.py -rfc $n $f $b $DATA
        done
    done
done
...
for n in 50 100; do
    for r in 0.1 1.0; do
        for d in 1 2; do
            $PROGRAM main.py -abc $n $r $d $DATA
        done
    done
done
...
for v in 1e-9 1e-5; do
    $PROGRAM main.py -gnb $v 0 0 $DATA
done
...
for c in 0.1 1.0 10.0; do
    for l in hinge squared_hinge; do
        for t in 1e-4 1e-3; do
            $PROGRAM main.py -lsvc $c $l $t $DATA
        done
    done
done
...
for h in "50" "100,50"; do
    for a in relu tanh; do
        for r in 0.0001 0.05; do
            $PROGRAM main.py -mlp "$h" $a $r $DATA
        done
    done
done
...
for l in hinge log_loss; do
    for p in l1 l2 elasticnet; do
        for r in constant adaptive; do
            $PROGRAM main.py -sgd $l $p $r $DATA
        done
    done
done
...
for n in 100 200; do
    for d in 10 20; do
        for s in 2 10; do
            $PROGRAM main.py -etc $n $d $s $DATA
        done
    done
done
